...
The root `plan.md`, `questions.md`, and `vision.md` are historical context, not the current contract. `scf-export-2026-07-05.json` is also historical. The deployed July 23 export is committed as `scf-export-2026-07-23.json` and has been audited as compatible with Plan 02 for active names/types, seven repeater children, `d/m/Y` dates, SCF REST exposure, and taxonomy registrations/default REST slugs. Live rollout still requires a manual one-post check of the effective release-type category ID and SCF's `default_to_current_date` behavior.
